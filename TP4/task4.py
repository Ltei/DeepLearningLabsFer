...
layer_e2 = vae_layer(layer_e1, n_hidden_recog_1, n_hidden_recog_2, "layer_e2")  # TODO
...
with tf.name_scope('z'):
    ...
    z_sigma = vae_layer(layer_e2, n_hidden_recog_2, n_z, 'z_log_sigma_sq', act=tf.identity)  # TODO
    z_sigma_sq = z_sigma * z_sigma
    z_log_sigma_sq = tf.log(z_sigma_sq)
...
layer_d2 = vae_layer(layer_d1, n_hidden_gener_1, n_hidden_gener_2, "layer_d2")  # TODO

# Define the mean value of the reconstruction
x_reconstr_mean = vae_layer(layer_d2, n_hidden_gener_2, n_input, 'layer_decoder', act=tf.identity)  # TODO
...
with tf.name_scope('cost'):
    # Reconstruction loss
    cost1 = tf.nn.sigmoid_cross_entropy_with_logits(logits=x_reconstr_mean, labels=x)  # TODO
    tf.summary.histogram('cross_entropy', cost1)
    # Latent loss
    cost2 = -0.5 * (1 + z_log_sigma_sq - tf.square(z_mean) - z_sigma_sq)  # TODO
